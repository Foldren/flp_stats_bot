...
class Maybank:
        ...
        for page in self.pages:
            ...
            for i in range(1, row_count):
                payment_accounts.append({
                    "number": await table.locator('tr').nth(i).locator('td').nth(0).locator(
    ...
    async def __get_transactions(self, pages_pas: list[list[dict[str, str]]]):
                    ...
                    while have_next_button:
                        ...
                        if row_count > 1:
                            for k in range(1, row_count):
                                ...
                                try:
                                    pas_statements[payment_account["number"] + " " + payment_account["currency"]].append(transaction)
                                except KeyError:
                                    pas_statements[payment_account["number"] + " " + payment_account["currency"]] = [transaction]
        ...
        print(json.dumps(pas_statements, indent=4))
